...
int main() 
{
	...
	while (iJob == JOB_NONE)
	{
		...
		if (cin.fail())
		{
			...
			iJob = JOB_NONE;
		// 1 ~ 3 사이에서 택하지 않은 경우 잡논을 넣어서 다시 선택 무한반복.
		// 가둬버리귀
	}
	...
	// 인트 형식인 iJob의 값을 형변환해서 eJob에 넣어줌.
	// 애초에 열거체 목록 타입으로 만든 구조체 변수이기에 int 타입이 아니라 이렇게 해주지않으면
	// 오류가 난다. cpp11부터는 enum class로 더 명확하게 정해줄 수 있지만
	// 열거체명을 일일히 붙여줘야하는게 귀찮고 아직 enum으로 쓰는곳이 더 많은 듯.
	tPlayer.tInventory.iGold = 60000;
	...
	while (true)
	{
		...
		case MM_MAP:
			while (true)
			{
				...
				{
					system("cls");
					//cls 해주지않으면 전투중 아래 문구가 이전문구까지 계속해서 나온다.
					switch (iMenu)
					{
					case MT_EASY:
						cout << "************************** 쉬움 **************************" << endl;
					...
					case BATTLE_ATTACK:
					{
						...
						if (tPlayer.bEquip[EQ_ARMOR])
						{
							iArmorMin += tPlayer.iArmorMin;
							iArmorMax += tPlayer.iArmorMax;
						}
						...
						system("pause");
						//pause 해주지않으면 위 cls 때문에 피해를 입힌 문구가 안나온다.
					}
						break;
					}
				}
		...
		case MM_INVENTORY:
			while (true)
			{
				...
				else
				{
					...
					{
						tPlayer.tInventory.tItem[idx] = tPlayer.tInventory.tItem[idx + 1];
					}
					--tPlayer.tInventory.iItemCount;
