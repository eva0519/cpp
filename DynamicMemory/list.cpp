...
노드를 새로 추가할때는 노드를 생성하고 마지막 노드에 연결만 해주면 되기 떄문에 개수의 제한이
없다.
*/
...
void InputString(char* pString, int iSize)
{
	cin.clear();
	cin.ignore(1024, '\n');  
	// 이전 메뉴 선택시 cin을 사용해 버퍼에 엔터가 남아있으니 지워야한다.
	// getline 함수 사용 이전에 cin이 사용되었다면 그냥 마음 편하게 버퍼내용을 다 비워주자.
	cin.getline(pString, iSize - 1); // 마지막 1바이트는 null이 들어가야하기 때문에 -1
}

void InitList(PLIST pList)
{
	// 포인터는 가급적이면 초기화할때 NULL(0) 으로 초기화해두고 쓰는 것이 좋다.
	// 왜냐하면 0은 false 0이 아닌 모든 수는 true이기 떄문이다.
	// 초기화를 하지 않을 경우 쓰레기값이 들어가있는데 이 쓰레기값 조차 true이다.
	pList->pBegin = NULL;
	pList->pEnd = NULL;
	pList->iSize = 0;
}
