...
// tEquip, bEquip 이 부분에 맥스를 대입하는 이유는 다른 장착 부위가 추가될수
// 있고 그렇다면 확장이 용이하기 때문이다. 
// 소리가 안나와서 모르겠지만 이 외에는 이유가 없다.
// tEquip은 장착할 수 있는 장비 구조체의 종류가 늘어나는 것이고.
// bEquip은 그 장비를 장착했는지 여부확인을 위해 {true, false, false, true, false}
// 이런 느낌의 형식을 만들기 위해 사용었다.
// 즉 장비할 수 있는 장비 종류 수를 늘리고 싶다면 아이템 타입과
// EQUIP enum에 EQ_MAX 위에 추가적인 장비 타입을 추가해주면 된다.
...
void SetPlayer(_tagPlayer* pPlayer)
{
	...
	pPlayer->iLevel = 1;
	pPlayer->iExp = 0;
	pPlayer->tInventory.iGold = 60000;
...
{
	//플레이어 정보를 출력한다. 
	//pPlayer는 배열이 아니니 포인터 역참조 간단문법으로 내부멤버변수의 값을 수정.
	cout << "************************** Player **************************" << endl;
...
void Battle(_tagPlayer* pPlayer, _tagMonster* pMonster)
{
	...
	if (pMonster->iHP <= 0)
	{
		...
		if (pPlayer->iExp >= g_iLevelUpExp[pPlayer->iLevel - 1])
		{
			...
			// 능력치를 상승시킨다. 
			// 직업 인덱스를 구한다.
			// ejob에 넣은 직업조건은 1기사~3법사이지만 
			// 레벨업 테이블 인덱스는 0~2 순으로 기사 법사이므로 
			// ejob에 -1을 빼줘서 인덱스로 넣는다.
	...
	// 방어구를 착용하고 있다면 방어구의 방어력을 더해준다.
	if (pPlayer->bEquip[EQ_ARMOR])
	{
		iArmorMin += pPlayer->iArmorMin;
		iArmorMax += pPlayer->iArmorMax;
	}
	...
	int iItemCount, int iStoreType)
{
	while (true)
	{
		...
		// 처음에 iItemCount 하나도 추가되어있지 않기 때문에 0으로 초기화
...
_tagItem CreateItem(char* pName, ITEM_TYPE eType, int iMin, int iMax, int iPrice,
...
void RunInventory(_tagPlayer* pPlayer)
{
	while (true)
	{
		...
		// 결정하게 한다.
		EQUIP	eq = ComputEquipType(pPlayer->tInventory.tItem[idx].eType);
		...
		else
		{
			...
			// 프레스 기능. 인덱스의 데이터들을 앞으로 당긴다.
			for (int i = idx; i < pPlayer->tInventory.iItemCount - 1; i++)
			{
				pPlayer->tInventory.tItem[idx] = pPlayer->tInventory.tItem[idx + 1];
			}
			--pPlayer->tInventory.iItemCount;

			// 장착을 했기 때문에 true 만들어준다. !중요!
			pPlayer->bEquip[eq] = true;
			// 무기를 장착했다 가정한다면, 플레이어 객체를 만들때 전체 초기화로 0이기 때문에
			// 내부는 {false, false} 로 되어있을것이고 {true, false}로 바뀔 것이다.
			// 그렇다면 이 else 문 이전 위 if문에서 걸러지게 된다.
		}
...
int main()
{
	...
	g_tLvUpTable[JOB_WIZARD - 1] = CreateLvUpStatus(15, 20, 3, 5, 20, 40, 50, 100);
