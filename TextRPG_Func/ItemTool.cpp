...
void AddItem(_tagItem* pWeaponStore, _tagItem* pArmorStore)
{
	...
	while (true)
	{
		...
		// 1부터 시작이기 때문에 1을 감소시켜야 상점 인덱스가 된다.
		--iItemLevel;
		break;
	}
	...
	// 인자로 무기상점에서 사용할 아이템 배열과 방어구 상점에서
	// 사용할 아이템 배열을 따로 넣어주었다. 아이템을 추가할때에도
	// 추가할 상점을 선택하고 아이템의 난이도를 선택했다.
...
int main()
{
	...
	_tagItem tArmor[STORE_WEAPON_MAX] = {};
