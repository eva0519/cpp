...
// 전역변수는 소속된 코드블럭이 없으므로 프로그램이 종료될때까지 메모리에 존재하게
...
함수명 : 함수의 이름을 의미한다. 함수명은 가급적이면 이 함수가 히는 역할을
정확하게 표시해 주는 것이 코드 가독성에 좋다.
인자 : 해당 함수를 사용할때 넘겨줄 값이 있을 경우 인자를 활용한다. 인자는 있을수도
있고 없을수도 있다.
...
// 리턴값이 없을 경우 void 반환타입을 사용한다.
// 그외 타입의 경우 리턴값이 없을 경우 오류가 나게 된다.
...
//함수 오버로딩시 디폴트인자를 사용할 때 위와 아래 같은 경우는 모호해지므로 주의. 
//void Output(int iNum1)
//{
//}

/*
함수의 오버로딩 : 같은 이름의 함수지만 기능이 다르던가 인자가 다른 함수를
말한다.
오버로딩의 조건 : 함수 이름이 무조건 같아야 한다. 단, 인자의 개수 혹은 타입이
달라야 오버로딩이 성립이 된다. 반환타입은 오버로딩에 영향을 주지 않는다.
*/

// 다른 함수에 있는 변수 iNumber를 바꾸고 싶으면 바꾸고 싶은 변수의 주소 &을
// 포인터 변수 인자로 받아와서 역참조로 바꾼다.
...
int main()
{
	...
	// iNum1은 위의 코드블럭 안에 존재하는 변수이므로 저 코드블럭 안에서만
	// 사용할 수 있다.
	/*cout << iNum1 << endl;*/
